...
\begin{definition}[Flat termination]
    ...
    \begin{itemize}
        ...
        \item If $x = f(x_1, \cdots, x_n)$ for some $f \in \Sigma_{def}$, or if $x \in V$, then $x^\flat := v^\flat$, where $v^\flat$ is some unused variable in $V^\flat$. 
    \end{itemize}
...
\begin{theorem}
    Consider some TRS $P$. If $P$ is size-change terminating with respect to the subterm ordering $\subterm$, then it is also flat terminating.
\end{theorem}
\begin{proof}
